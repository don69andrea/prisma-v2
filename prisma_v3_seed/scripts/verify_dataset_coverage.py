...
_NUM_FIELDS = ["pe_ratio", "roe", "debt_equity", "fcf_margin", "eps_chf",
               "dividend_yield", "market_cap_chf"]
...
async def _probe_fmp(tickers): return {t: (0, 1.0) for t in tickers}      # TODO: fmp_adapter
async def _probe_simfin_us(tickers): return {t: (0, 1.0) for t in tickers}  # TODO: simfin_adapter (market='us')
...
async def main() -> None:
    ...
    lines += ["", f"## Empfehlung: **{recommended or 'KEINE Quelle besteht — Plan B (US-Proxy) wählen'}**", ""]
    if recommended:
        lines.append(f"→ in backend/config.py `dataset_source_fundamentals` entsprechend setzen.")
    else:
