...
def validate_ohlcv(rows: list[dict], *, table: str, spike_pct: float = 0.25) -> tuple[list[dict], ValidationReport]:
    ...
    for r in rows:
        ...
        if prev_close is not None and abs(c - prev_close) / prev_close > spike_pct:
            spikes.append(f"{r['ticker']} {r.get('date') or r.get('timestamp')}: "
                          f"{prev_close:.2f}->{c:.2f}")
        clean.append(r)
        prev_close = c
        prev_key = r.get("date") or r.get("timestamp")
    rep = ValidationReport(table=table, rows_in=len(rows), rows_out=len(clean),
                           dropped=dropped, spikes=spikes, gaps=gaps)
    if dropped:
        log.warning("validate %s: %d Zeilen verworfen (von %d)", table, dropped, len(rows))
    if spikes:
        log.warning("validate %s: %d Preis-Spikes >%.0f%%: %s",
                    table, len(spikes), spike_pct * 100, spikes[:5])
    return clean, rep
