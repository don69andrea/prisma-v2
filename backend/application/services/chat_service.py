...
async def _get_macro_context(inputs: dict[str, Any], session: AsyncSession) -> str:
    ...
    return json.dumps(
        {"snb_rate": ctx.leitzins, "chf_eur": ctx.chf_eur, "inflation_ch": ctx.inflation_ch}
    )
...
async def _get_ranking(inputs: dict[str, Any], session: AsyncSession) -> str:
    ...
    ranked = sorted(stocks, key=lambda s: getattr(s, "quant_score", None) or 0, reverse=True)[
        :top_n
    ]
    return json.dumps(
        [
                ...
                for block in final.content:
                    if block.type == "tool_use":
                        ...
                        if len(result_str) > _TOOL_RESULT_MAX_CHARS:
                            _logger.debug(
                                "Tool '%s' Ergebnis auf %d Zeichen gekürzt (war %d)",
                                block.name,
...
async def _dispatch_tool(name: str, inputs: dict[str, Any]) -> str:
    ...
    if handler is None:
        return json.dumps(
            {"error": f"Unbekanntes Tool '{name}'. Bekannte Tools: {list(_TOOL_REGISTRY)}"}
        )
