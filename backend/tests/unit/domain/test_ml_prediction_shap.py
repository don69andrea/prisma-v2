...
def test_shap_entry_fields() -> None:
    entry = SHAPEntry(feature="roe_zscore", shap_value=0.3, feature_value=1.2, label="Return on Equity")
...
def test_ml_prediction_has_shap_fields() -> None:
    pred = MLPrediction(
        ...
        shap_values=[SHAPEntry(feature="roe_zscore", shap_value=0.3, feature_value=1.2, label="Return on Equity")],
...
def test_shap_values_not_shared_between_instances() -> None:
    ...
    pred_b = MLPrediction(
        ticker="NOVN", snapshot_date=date(2026, 1, 1), predicted_class=1, signal="NEUTRAL",
        prob_bottom=0.2, prob_mid=0.6, prob_top=0.2, confidence=0.6, model_type="xgboost", features={},
    )
