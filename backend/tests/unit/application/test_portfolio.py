...
class TestDrawdownBrake:
    def test_drawdown_brake_halves_exposure(self) -> None:
        ...
        vols = {"BTC-USD": 0.60}
        normal = allocate_portfolio(
            signals, vols, eligible_coins=frozenset(signals), portfolio_dd=0.0
        )
        braked = allocate_portfolio(
            signals, vols, eligible_coins=frozenset(signals), portfolio_dd=-0.20
        )
    ...
    def test_no_brake_within_threshold(self) -> None:
        ...
        just_ok = allocate_portfolio(
            signals, vols, eligible_coins=frozenset(signals), portfolio_dd=-0.14
        )
...
class TestEligibilityFilter:
    def test_ineligible_coin_excluded(self) -> None:
        ...
        result = allocate_portfolio(signals, vols, eligible_coins=frozenset({"BTC-USD"}))
